...
def send_email():
    ...
    try:
        ...
        for prod in productList:
            totalPrice = float(prod["price"]) * int(prod["qty"])
            productHTML += "<td>" + prod["itemName"] + "</td><td>" + str(prod["qty"]) + "</td><td>" + str(prod["price"]) +  "</td><td>" + str(totalPrice) + "</td></tr>"
        data = {
            'Messages': [
                {
                "From": {
